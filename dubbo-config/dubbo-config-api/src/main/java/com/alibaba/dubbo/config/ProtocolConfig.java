...
public class ProtocolConfig extends AbstractConfig {
    ...
    // service port
    /*
     * port	<port>	int	可选	dubbo协议缺省端口为20880，
     * rmi协议缺省端口为1099，
     * http和hessian协议缺省端口为80；
     * 如果配置为-1 或者 没有配置port，则会分配一个没有被占用的端口。
     * Dubbo 2.4.0+，分配的端口在协议缺省端口的基础上增长，确保端口段可控。	服务发现	服务端口
     * */
    ...
    // thread pool
    /*
     * threadpool	threadpool	string	可选	缺省值 fixed
    ...
    /*
     * threads	threads	int	可选	缺省值100 性能调优	服务线程池大小(固定大小)
     * */
    private Integer threads;

    // IO thread pool size (fixed size)
    /*
     * iothreads	threads	int	可选 缺省值cpu个数+1	性能调优	io线程池大小(固定大小)
     * */
     ...
     * 性能调优	线程池队列大小，当线程池满时，排队等待执行的队列大小，建议不要设置，
    ...
    /*
     * accepts	accepts	int	可选	缺省值 0	性能调优	服务提供方最大可接受连接数
     * */
    private Integer accepts;

    // protocol codec
    /*
     * codec	codec	string	可选	缺省值 dubbo	性能调优	协议编码方式
     * */
    private String codec;

    // serialization
    /*
     * serialization	serialization	string	可选	dubbo协议缺省为hessian2，
    ...
    // payload max length
    /*
     * payload	payload	int	可选 缺省值 88388608(=8M)	性能调优	请求及响应数据包大小限制，单位：字节
     * */
    ...
    // heartbeat interval
    /*
     * heartbeat	heartbeat	int	可选	0	性能调优	心跳间隔，对于长连接，当物理层断开时，
     ...
     * 性能调优	协议的服务器端实现类型，比如：dubbo协议的mina,netty等，http协议的jetty,servlet等
     * */
